...
resource "local_file" "cert_key_pem" {
  content  = module.supermicro_certificate.private_key_pem
  filename = "${path.module}/${var.folder_name}/supermicro_key.pem"
}

# Сертификат Proxmox 5 (proxmox5.crt)
resource "local_file" "cert_pem" {
  content  = module.supermicro_certificate.cert_pem
  filename = "${path.module}/${var.folder_name}/supermicro_cert.pem"
}
